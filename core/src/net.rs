...
use bevy::prelude::*;
use bevy_renet::{netcode::{ClientAuthentication, NetcodeClientTransport, NetcodeServerTransport, ServerAuthentication, ServerConfig}, renet::{ChannelConfig, ClientId, ConnectionConfig, RenetClient, RenetServer, SendType}};
        ...
        WorldSnapshot {
            ...
            character_entities: {
                ...
                for entity in self.character_entities.iter() {
                    if let Some(other_entity) = other.character_entities.iter().find(|e| e.id == entity.id) {
...
fn is_nearly_equal_vec3(a: Option<Vec3>, b: Option<Vec3>) -> bool {
    match (a, b) {
        (Some(a), Some(b)) => is_nearly_equal(a.x, b.x) 
            && is_nearly_equal(a.y, b.y) 
            && is_nearly_equal(a.z, b.z),
        (None, None) => true,
        _ => false,
    }
}
